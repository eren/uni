...
(define (calc an-ae [fundefs empty])
  (type-case AE an-ae
    [ae-num (n) n]
    [ae-add (l r) (+ (calc l fundefs) (calc r fundefs))] 
    [ae-sub (l r) (- (calc l fundefs) (calc r fundefs))]
    [ae-mul (l r) (* (calc l fundefs) (calc r fundefs))]
    [ae-div (l r) (/ (calc l fundefs) (calc r fundefs))]
    [ae-expt (l r) (expt (calc l fundefs) (calc r fundefs))]
    [ae-with (bound-id named-expr bound-body)
             (calc (subst bound-body (ae-id-s bound-id)
                          (ae-num (calc named-expr fundefs)))
                   fundefs)]
    [ae-app (fun-name arg-expr)
            ...
            (local ([define the-fun-def (lookup-fundef fun-name fundefs)])
              (calc (subst (fundef-body the-fun-def)
                           ...
                           (ae-num (calc arg-expr fundefs)))
                    fundefs))]
    [ae-id (s) (error 'calc "free identifier ~a" s)]
    )
  )
